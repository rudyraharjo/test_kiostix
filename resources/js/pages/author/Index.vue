                    ...
                    <v-dialog v-model="dialogDelete" persistent max-width="290">
                        <v-card>
                            ...
                            <v-card-actions>
                                ...
                                <!-- 
                                <v-btn color="green darken-5" @click="dialogDelete = false">Cancel</v-btn>
                                <v-btn color="green darken-1" :loading="btnLoading" @click="deleteItem(itemTemp, 'ok')">OK</v-btn>
                                -->
                            </v-card-actions>
                        </v-card>
                    </v-dialog>
        ...
        methods: {
            ...
            save () {
                ...
                if (this.editedIndex > -1) {
                    ...
                    this.$store.dispatch('author/ACT_UPDATE', this.editedItem)
                    .then(res =>{
                        toast.fire({
                            type: 'success',
                            title: "Success Updated Author"
                        });
                    })
                    .catch(err => {
                        toast.fire({
                    ...
                    this.$store.dispatch('author/ACT_SAVE', this.editedItem)
                    .then(res =>{
                        toast.fire({
                            type: 'success',
            ...
            editItem(item){
                ...
                this.editedIndex = this.itemauthors.indexOf(item)
                this.editedItem = Object.assign({}, item)
                this.dialog = true
            },
            deleteItem(item, value) {
                const deleteIndex = this.itemauthors.indexOf(item)
                this.itemTemp = item
                if(value == "ok"){
                    this.btnLoading = true
                    this.$store.dispatch('author/ACT_DELETED', {
                        index: deleteIndex,
                        id:item.id
                    })
                        .then(res =>{
                ...
                // confirm('Are you sure you want to delete this item?') && this.itemauthors.splice(index, 1)
            },
            close () {
                this.dialog = false
                setTimeout(() => {
